    ...
    <section>
       ...
       <ui-prop name="${Editor.T('KEYSTORE.name')}" v-bind:error="commonNameError">
        ...
        <ui-prop name="Email Address" v-bind:error="emailError">
         ...
         <ui-prop name="${Editor.T('huawei-runtime.save_certificate_path')}" v-bind:error="certificatePathError">
        <ui-input v-value="certificatePath" class="flex-1" placeholder="${Editor.T('huawei-runtime.select_save_certificate_path')}"></ui-input>
        ...
        window.abc = this._vm = new window.Vue({
            ...
            methods: {
                ...
                _onSaveClick(event) {
                    ...
                    // var certificateCmd = 'openssl req -newkey rsa:2048 -nodes -keyout private.pem -x509 -days 3650 -out certificate.pem -subj ' + '"' + subjString + '"';
                    ...
                    exec(`${certificateCmd}`, {
                        ...
                        that.saving = false;
                        if (!error) {
                            Editor.log(Editor.T('huawei-runtime.build_certificate_complet'));
                            Editor.Ipc.sendToWins('builder:events', 'certificate-created', dir);
                            Editor.Panel.close('huawei-runtime');
                        }
                        else {
                            //这里需要增加windows openssl是否安装
                            Editor.error(Editor.T('huawei-runtime.build_certificate_fail') + error);
                            return;
                        }
                    });
