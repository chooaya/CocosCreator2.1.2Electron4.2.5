    ...
    ready() {
        ...
        window.abc = this._vm = new window.Vue({
            ...
            methods: {
                ...
                _isInstallOpenSSL(complete) {
                    var isWin32 = process.platform === 'win32';
                    var exec = require('child_process').exec;
                    var cmd = isWin32 ? `where openssl` : `which openssl`;
                    exec(`${cmd}`, {}, (error) => {
                        if (!error && complete) {
                            complete();
                            return
                        ...
                        if (isWin32) {
                            Editor.error(`生成证书需要openssl,请先安装openssl并配置环境变量,下载地址http://slproweb.com/products/Win32OpenSSL.html`);
                        } else {
                ...
                _onSaveClick(event) {
                    ...
                    exec(`${certificateCmd}`, {
                        ...
                        that.saving = false;
                        if (error) {
                            Editor.error(Editor.T('oppo-runtime.build_certificate_fail') + error);
                            return;
                        }
